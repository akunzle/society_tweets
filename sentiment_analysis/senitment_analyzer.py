...
tweets_stop_words_removed = [tokenize_and_remove_stop_words(tweet, language = 'spanish') for tweet in cleaned_tweets]
...
tweets_stemmed = [tokenize_and_stem(tweet, language = 'spanish') for tweet in cleaned_tweets]
...
tweets_mark_negation = [mark_negation_es(tweet) for tweet in cleaned_tweets]
...
# loop through each text and each word in the text to lemmatize it
for text in cleaned_tweets:
    for word in text.split():
        lemma = analyzer.lemmatize_spa(word)
        print(word, ":", lemma)

polarity_score = [analyzer.spa_polarity_score(text) for text in cleaned_tweets]
print(polarity_score)

analyzed_tweets = [analyzer.analyze_doc(text) for text in cleaned_tweets]
...
ctg = DocumentClustering(min_df=0.01, max_df=0.8, language='spanish')
...
clusters_docs = clusters.docs
features = clusters.features
num_docs_p_cluster = clusters.num_docs_per_cluster
print(clusters_docs)
print(features)
...
# Open the input file and read the tweet texts and post dates
with open(output_file, 'r', newline='', encoding='utf-8') as file:
    reader = csv.reader(file)
    # Skip the header row
    next(reader)
...
# Analyze the sentiment of the cleaned tweet texts
analyzer = SentimentAnalyzer(language='spanish')
analyzed_tweets = [analyzer.analyze_doc(text) for text in cleaned_tweets]

# Open the output file and write the results
with open("algoritmo_tweets_analysis.csv", 'w', newline='', encoding='utf-8') as file:
    writer = csv.writer(file)
    # Write the header row
    writer.writerow(['postDates', 'tweets', 'sentiment_label', 'polarity_score'])
    # Iterate through each row and write the results
    for i in range(len(tweet_dates)):
        writer.writerow([tweet_dates[i], cleaned_tweets[i], analyzed_tweets[i][1], analyzed_tweets[i][2]])
...
min_time_str = str(datetime.utcfromtimestamp(min_time_between_tweets))[:-3]
max_time_between_tweets = max(time_between_tweets)
max_time_str = str(datetime.utcfromtimestamp(max_time_between_tweets))[:-3]

# Calculate sentiment analysis stats
positive_tweets = len([tweet for tweet in analyzed_tweets if tweet[1] == 'pos'])
neutral_tweets = len([tweet for tweet in analyzed_tweets if tweet[1] == 'neu'])
negative_tweets = len([tweet for tweet in analyzed_tweets if tweet[1] == 'neg'])
pos_mean_polarity = sum([scores[2] for scores in analyzed_tweets if scores[1] == 'pos']) / positive_tweets
neu_mean_polarity = sum([scores[2] for scores in analyzed_tweets if scores[1] == 'neu']) / neutral_tweets
neg_mean_polarity = sum([scores[2] for scores in analyzed_tweets if scores[1] == 'neg']) / negative_tweets
...
with open('algoritmo_tweet_stats.json', 'w') as f:
