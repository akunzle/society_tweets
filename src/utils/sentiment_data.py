...
logging.basicConfig(filename=str(pathlib.Path.cwd().joinpath('society_tweets.log')), level=logging.DEBUG)
...
client = MongoClient('mongodb+srv://askm:Maciel1402!@cluster88539.allnb4n.mongodb.net/test?authSource=admin&appName=mongosh+1.8.2')
db = client['society_tweets']
collection = db['algoritmo_tweets']

# Reading CSV file
with open('/Users/andreakunzle/society_tweets/sentiment_analysis/algoritmo_tweets.csv', 'r') as csvfile:
...
from src.utils.society_db_manager import DBManager
...
logging.basicConfig(filename=str(pathlib.Path(__file__).parents[1].joinpath('society_tweets.log')), level=logging.DEBUG)
...
data_file = Path("/Users/andreakunzle/society_tweets/sentiment_analysis/algoritmo_tweets.csv")
df = pd.read_csv(data_file)

# Initialize an instance of the SentimentAnalyzer class
